...
class UserImporter extends \TYPO3\CMS\Backend\Module\BaseScriptClass
{
    ...
    function main()
    {
        ...
        $this->doc->JScode = '
            ...
            ((array_key_exists(
                    'userGroupMandatoryFE',
                    $this->extensionConfiguration
                ) && (boolean)$this->extensionConfiguration['userGroupMandatoryFE'] === true && ($this->inData['settings']['importUserType'] == 'FE' || !($this->inData['settings']['importUserType']))) ?
        ...
        $docHeaderButtons = [
            'SHORTCUT' => $this->getBackendUserAuthentication()->mayMakeShortcut() ? $this->doc->makeShortcutIcon(
                "id",
        ...
        $markers['FUNCMENU'] = $this->doc->funcMenu(
            '',
            BackendUtility::getFuncMenu($this->id, "SET[function]", $this->MOD_SETTINGS['function'], $this->MOD_MENU['function'])
        );
    ...
    protected function moduleContent()
    {
        ...
        if ($newFileObjectOrErrorMessage instanceof File && !file_exists($newFileObjectOrErrorMessage->getForLocalProcessing(false))) {
            $this->content .= $this->doc->section(
                $GLOBALS['LANG']->getLL('f1.tab1.section.error'),
                $GLOBALS['LANG']->getLL('f1.tab1.section.error.description'),
                0,
                1,
                3
            );
                ...
                if (is_array($this->inData['rollback'])) {
                    ...
                    foreach ($candidates as $user) {
                        ...
                        if ($rollbackDeleteFromDB) {
                            ...
                            $this->getBackendUserAuthentication()->writelog(1, 3, 0, '', 'UID %s deleted by CSV rollback action', [$user]);
                        } else {
                            $GLOBALS['TYPO3_DB']->exec_UPDATEquery($data['db_table'], 'uid=' . $user, ['deleted' => '1']);
                        }
                    }
                    $GLOBALS['TYPO3_DB']->exec_UPDATEquery('tx_rsuserimp_sessions', 'uid=' . $rollbackSession[0], ['active' => '0']);
                }
                ...
                if (!empty($rollbackData)) {
                    ...
                    foreach ($rollbackData as $session) {
							...
							<div align="left">
								<table cellpadding="0px" cellspacing="0px" border="0">
									...
									<tr>
										...
										<td width="10pt"></td>
										<td>' . $session['num_drop'] . (!empty($session['dropfile']) && is_file(
                                $session['dropfile']
                            ) ? ' [<a href="/uploads/tx_rsuserimp/' . basename($session['dropfile']) . '">' . $GLOBALS['LANG']->getLL(
                                    'f1.tab5.downloadFile'
                                ) . '</a>]' : '') . '</td>
									</tr>
								</table>
							</div>';
                        ...
                        $infoBlock .= '<div align="right">
													<input type="submit" onclick="return confirm(\'' . $GLOBALS['LANG']->getLL(
                                'f2.rollback.sure',
                                1
                            ) . '\');" name="tx_rsuserimp[rollback][' . $session['uid'] . ']" value="Roll back" ' . ((((mktime(
                                        ) < $rollbacktime) || $rollbackSafetyTimespan == 0) && $session['active'] == 1) ? '' : 'disabled') . '/>
												</div>';
                        ...
                        if ($now < $rollbacktime || $rollbackSafetyTimespan == 0) {
                            if ($session['active'] == 1) {
                                $content .= '<td valign="top">' . sprintf(
                                        $GLOBALS['LANG']->getLL('f2.sessionRollback'),
                                        BackendUtility::dateTimeAge($rollbacktime, 1)
                                    ) . '</td>';
                            }
                            ...
                            $content .= '<td valign="top">' . sprintf(
                                    $GLOBALS['LANG']->getLL('f2.sessionExpired'),
                                    BackendUtility::dateTimeAge($rollbacktime, 1)
                                ) . '</td>';
                        }
            ...
            case 1:
                ...
                $row[] = '
                    ...
                    $GLOBALS['LANG']->getLL('importFile') . '
								...
								<input type="hidden" name="file[upload][1][target]" value="' . htmlspecialchars(
                        $tempDir
                    ) . '" ' . ($_POST['importNow'] ? 'disabled' : '') . '/>
                ...
                $row[] = '
							<tr class="db_list_normal">
								...
								<td>' . $this->renderSelectBox(
                        'tx_rsuserimp[settings][importUserType]',
                        (isset($this->inData['settings']['importUserType']) ? $this->inData['settings']['importUserType'] : ''),
                        $opt,
                        '',
                        "onchange='submit()'"
                    ) . '</td>
                ...
                if ($mapper->userType == 'FE') {
                    ...
                    foreach ($preopt as $val) {
                        $opt = $opt + [$val['uid'] => $val['title'] . ' [UID ' . $val['uid'] . ']'];
                    }
                } else {
                ...
                $row[] = '
							<tr class="db_list_normal">
								...
								<td>' . $this->renderSelectBox(
                        'tx_rsuserimp[settings][importStorageFolder]',
                        (isset($this->inData['settings']['importStorageFolder']) ? $this->inData['settings']['importStorageFolder'] : ''),
                        $opt,
                        'importStorageFolder'
                    ) . '</td>
							</tr>';

                if (empty($this->inData) || (is_array($this->inData) && is_array(
                            $this->inData['settings']
                        ) && $this->inData['settings']['importUserType'] === 'FE')) {
                    ...
                    if (!empty($preopt)) {
                        ...
                        $opt = ['' => $GLOBALS['LANG']->getLL('f1.tab2.section.defaultGroup.emptyGroup')];
                    }
                    $row[] = '
								<tr class="db_list_normal">
									<td><strong>' . $GLOBALS['LANG']->getLL('f1.tab2.section.defaultGroup.label') . '</strong></td>
									<td>' . $this->renderMultipleSelector(
                            'tx_rsuserimp[settings][importUserGroup]',
                            $opt,
                            (isset($this->inData['settings']['importUserGroup']) ? $this->inData['settings']['importUserGroup'] : ''),
                            1,
                            'importUserGroup'
                        ) . '</td>
								</tr>';
                ...
                $row[] = '
							<tr class="db_list_normal">
								...
								<td>' . $this->renderSelectBox(
                        'tx_rsuserimp[settings][uniqueIdentifier]',
                        (isset($this->inData['settings']['uniqueIdentifier']) ? $this->inData['settings']['uniqueIdentifier'] : ''),
                        $opt,
                        'uniqueIdentifier',
                        ''
                    ) . '</td>
                ...
                $opt = [
                    ';' => $GLOBALS['LANG']->getLL('f1.tab2.constants.semicolon'),
                    ',' => $GLOBALS['LANG']->getLL('f1.tab2.constants.comma'),
                    ':' => $GLOBALS['LANG']->getLL('f1.tab2.constants.colon'),
                    'TAB' => $GLOBALS['LANG']->getLL('f1.tab2.constants.tab')
                ];
                $row[] = '
							<tr class="db_list_normal">
								<td><strong>' . $GLOBALS['LANG']->getLL('f1.tab2.section.generalSettings.fieldDelimiter') . '</strong></td>
								<td>' . $this->renderSelectBox(
                        'tx_rsuserimp[settings][fieldDelimiter]',
                        (isset($this->inData['settings']['fieldDelimiter']) ? $this->inData['settings']['fieldDelimiter'] : ''),
                        $opt
                    ) . '</td>
							</tr>';
                ...
                $row[] = '
							<tr class="db_list_normal">
								...
								<td>' . $this->renderSelectBox(
                        'tx_rsuserimp[settings][fieldEncaps]',
                        (isset($this->inData['settings']['fieldEncaps']) ? $this->inData['settings']['fieldEncaps'] : ''),
                        $opt
                    ) . '</td>
							</tr>';
                ...
                $opt = [
                    'iso-8859-1' => $GLOBALS['LANG']->getLL('f1.tab2.constants.iso-8859-1'),
                    'UTF-8' => $GLOBALS['LANG']->getLL('f1.tab2.constants.utf-8')
                ];
                ...
                $row[] = '
							<tr class="db_list_normal">
								...
								<td>' . $this->renderSelectBox(
                        'tx_rsuserimp[settings][inputEncoding]',
                        (isset($this->inData['settings']['inputEncoding']) ? $this->inData['settings']['inputEncoding'] : ''),
                        $opt
                    ) . '</td>
							</tr>';
                ...
                $row[] = '
							<tr class="db_list_normal">
								...
								<td><input type="text" name="tx_rsuserimp[settings][maxPreview]" value="' . htmlspecialchars(
                        ((isset($this->inData['settings']['maxPreview']) && ($this->inData['settings']['maxPreview'] >= 0)) ? $this->inData['settings']['maxPreview'] : '3')
                    ) . '" size="2" maxlength="2" ' . ($_POST['importNow'] ? 'disabled' : '') . '></td>
							</tr>';
                ...
                $row[] = '
							<tr class="db_list_normal">
								...
								<td>' . $this->renderMultipleSelector(
                        'tx_rsuserimp[settings][extraFields]',
                        $dbFieldsDefault,
                        (isset($this->inData['settings']['extraFields']) ? $this->inData['settings']['extraFields'] : '')
                    ) . '</td>
							</tr>';
                ...
                $row[] = '
							<tr class="db_list_normal">
								...
								<td><div align="right"><input  onclick="return checkForm()" type="submit" name="tx_rsuserimp[settings][OK]" value="' . $GLOBALS['LANG']->getLL(
                        'f1.tab2.section.generalSettings.update.update',
                        1
                    ) . '" ' . ($_POST['importNow'] ? 'disabled' : '') . '/></div></td>
							</tr>';
                ...
                if (isset($this->inData['settings']['OK'])) {
                    ...
                    $mapper->defaultUserData['usergroup'] = isset($this->inData['settings']['importUserGroup']) ? implode(
                        ',',
                        $this->inData['settings']['importUserGroup']
                    ) : '';
                    ...
                    if (isset($this->inData['settings']['enableUpdate']) && !empty($this->inData['settings']['uniqueIdentifier'])) {
                        $mapper->mandatoryFields = array_merge($mapper->mandatoryFields, [$this->inData['settings']['uniqueIdentifier']]);
    ...
    protected function checkUpload()
    {
        ...
        if ($httpHost != $refInfo['host'] && $this->vC != $this->getBackendUserAuthentication()->veriCode(
    ...
    public function makeSaveForm($inData, &$row)
    {
        ...
        $row[] = '
                ...
                'tx_rsuserimp[preset][select]',
                (isset($inData['preset']['select']) ?
                    // JavaScript to check fields swapPresetSelectFields()
                    $inData['preset']['select'] : ''),
                $opt,
                'swapfield'
            ) . '
						<br/>
						<input type="submit" value="' . $GLOBALS['LANG']->getLL(
                'f1.tab2.section.presets.load.load',
                1
            ) . '" name="tx_rsuserimp[preset][load]" ' . ($_POST['importNow'] ? 'disabled' : '') . '/>
						<input type="submit" value="' . $GLOBALS['LANG']->getLL(
                'f1.tab2.section.presets.load.delete',
                1
            ) . '" name="tx_rsuserimp[preset][delete]" onclick="return confirm(\'' . $GLOBALS['LANG']->getLL(
                'f1.tab2.section.presets.load.delete.sure',
                1
            ) . '\');" ' . ($_POST['importNow'] ? 'disabled' : '') . '/>
					</td>
				</tr>
				<tr class="db_list_normal">
					...
					<td>
						...
						<input type="text" name="tx_rsuserimp[preset][savetitle]" value="' . htmlspecialchars(
                isset($inData['preset']['savetitle']) ? $inData['preset']['savetitle'] : ''
            ) . '" ' . ($_POST['importNow'] ? 'disabled' : '') . '/><br/>' .
            ...
            '<div align="right"><input type="submit" value="' . $GLOBALS['LANG']->getLL(
                'f1.tab2.section.presets.save.save',
                1
            ) . '" name="tx_rsuserimp[preset][save]" onclick="return swapPresetSelectFields();" ' . ($_POST['importNow'] ? 'disabled' : '') . '/></div>
					</td>
				</tr>';
    }
    ...
    function renderMultipleSelector($prefix, $allValues, $postData, $reverse = 0, $id = '')
    {
        ...
        return '<select id="' . $id . '" name="' . $prefix . '[]" multiple="multiple" ' . (($_POST['importNow'] || ((isset($this->inData['settings']['importUserType']) && ($this->inData['settings']['importUserType'] == 'TT')) && $id === 'importUserGroup')) ? 'disabled' : '') . ' size="' . \TYPO3\CMS\Core\Utility\MathUtility::forceIntegerInRange(
                count($opt),
                2,
                6
            ) . '">' . implode('', $opt) . '</select>';
    ...
    function renderSelectBox($prefix, $value, $optValues, $id = '', $JSfunction = '')
    {
        ...
        return '<select ' . $JSfunction . ' id="' . $id . '" name="' . $prefix . '" ' . ($_POST['importNow'] ? 'disabled' : '') . '>' . implode(
                '',
                $opt
            ) . '</select>';
    }
        ...
        if (!empty($rollbackData)) {
            foreach ($rollbackData as $session) {
                ...
                if ($rollbackSafetyTimespan == 0) {
                    ...
                    if (TYPO3_DLOG) {
                        GeneralUtility::devLog(
                            'Age out timer for session ' . $session['uid'] . ' set to ' . strftime("%d.%m.%Y - %H:%M:%S", $ageOutTime),
                            'rs_userimp',
                            -1
                        );
                        GeneralUtility::devLog(
                            'Delete timer for session ' . $session['uid'] . ' set to ' . strftime(
                                "%d.%m.%Y - %H:%M:%S",
                                $garbageCollectionTriggerTimer + $session['crdate']
                            ),
                            'rs_userimp',
                            -1
                        );
                ...
                if (($ageOutTime < $now) && $session['active']) {
                    $GLOBALS['TYPO3_DB']->exec_UPDATEquery('tx_rsuserimp_sessions', 'uid=' . $session['uid'], ['active' => '0']);
        ...
        if (!empty($result)) {
            foreach ($result as $session) {
                if (($session['crdate'] + $garbageCollectionTriggerTimer) <= $now) {
                    $GLOBALS['TYPO3_DB']->exec_UPDATEquery('tx_rsuserimp_sessions', 'uid=' . $session['uid'], ['deleted' => '1']);
                    GeneralUtility::devLog('Deleted aged out session ' . $session['uid'], 'rs_userimp', 1);
                    if ($session['dropfile'] && GeneralUtility::validPathStr($session['dropfile']) && GeneralUtility::isFirstPartOfStr(
                            $session['dropfile'],
                            PATH_site . 'uploads/tx_rsuserimp/'
                        )) {
