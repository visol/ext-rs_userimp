...
class UserImporterService
{
    ...
    public function createImportForm()
    {
        $content = $this->getLanguageService()->getLL('f1.tab4.section.import.label') . '
				<div align="right">
					<input type="submit" name="importNow" value="' . $this->getLanguageService()->getLL(
                'f1.tab4.section.import.import',
                1
            ) . '" ' . (GeneralUtility::_POST('importNow') ? 'disabled' : '') . ' onclick="return confirm(\'' . $this->getLanguageService()->getLL(
                'f1.tab4.section.import.sure',
                1
            ) . '\');">
				</div>';
        return $content;
    ...
    public function createMappingForm()
    {
        ...
        if (!$this->inData['import']) {
            $content .= '<div align="right"><input type="submit" name="map" value="' . $this->getLanguageService()->getLL(
                    'f1.tab3.mapper.import'
                ) . '" ' . (GeneralUtility::_POST('importNow') ? 'disabled' : '') . '></div>';
            ...
            foreach ($this->columnNamesFromCSV as $key) {
                $content .= '<tr class="db_list_normal">' .
                    '<td>' . $i . '</td>' .
                    '<td>' . $key . '</td>';
                $content .= (($this->enableAutoValues) ? '<td><input ' . (GeneralUtility::_POST(
                        'importNow'
                    ) ? 'disabled' : '') . ' onclick="toggleOptions(' . $i . ')" type="checkbox" name="tx_rsuserimp[autoval][' . $i . ']" ' . (($this->inData[autoval][$i] == 'on') ? ' checked ' : '') . '/></td>' : '');
                $content .= $this->createSelector($i);
                $content .= '</tr>';
                $i++;
            }

            $content .= '</table><div align="right"><input type="submit" name="map" value="' . $this->getLanguageService()->getLL(
                    'f1.tab3.mapper.import'
                ) . '" ' . (GeneralUtility::_POST('importNow') ? 'disabled' : '') . '></div>';
            $content .= '</fieldset>';
        }
    ...
    public function evaluateMappingForm()
    {
        ...
        //delete empty values
        $n = array_diff($this->inData['fieldmap'], ['']);
        $org = $n;
        $x = count($n);
                ...
                if (!$this->inData['import']) {
                    foreach ($this->mandatoryFields as $mandatoryField) {
                        if (!in_array($mandatoryField, $this->inData['fieldmap'])) {
                            if ($mandatoryField === 'username' && array_key_exists(
                                    'useEMailAsUsername',
                                    $this->inData['settings']
                                ) && $this->inData['settings']['useEMailAsUsername'] == 1) {
                            ...
                            } elseif ($mandatoryField === 'password' && array_key_exists(
                                    'generatePassword',
                                    $this->inData['settings']
                                ) && $this->inData['settings']['generatePassword'] == 1) {
                    ...
                    if (!empty($mandatoryFieldError)) {
                        $this->importOK = false;
                        $content .= $this->getLanguageService()->getLL('f1.tab3.mapper.message.provideMandatory') . ' <b>' . implode(
                                ', ',
                                $mandatoryFieldError
                            ) . '</b>';
                    } else {
    ...
    protected function createSelector($x)
    {
        ...
        if (isset($this->enableAutoValues)) {
            ...
            $content .= '<input name="tx_rsuserimp[customValue][' . $x . ']" type="text"' . (GeneralUtility::_POST(
                    'importNow'
    ...
    {
        $box = '<select style="display: block" name="tx_rsuserimp[fieldmap][' . $n . ']" ' . ' size="1" ' . (GeneralUtility::_POST(
                'importNow'
            ) ? 'disabled' : '') . '>' . "\n";
    ...
    public function importUsers()
    {
        ...
        $res = $this->getDatabaseConnection()->exec_SELECTquery(
            $this->uniqueUserIdentifier . ',uid',
            $this->userTypeDBTable,
            'pid=' . $this->defaultUserData['pid']
        );
        ...
        foreach ($CSV as $row) {
            ...
            if ($this->enableAutoValues && is_array($this->inData['autoval'])) {
                ...
                foreach ($this->inData['autoval'] as $key2 => $val2) {
                    ...
                    $sub2[$n][$this->inData['fieldmap'][$key2]] = $this->generateCustomValue(
                        $users[$n][$this->inData['fieldmap'][$key2]],
                        $this->inData['customValue'][$key2],
                        $row
                    );
                }
...
//						$msg[] = $this->getLanguageService()->getLL('f1.tab5.error.renamed');
            ...
            if ($importUser) {
                if ($updateUser) {
                    ...
                    $res = $this->getDatabaseConnection()->exec_UPDATEquery(
                        $this->userTypeDBTable,
                        ($this->uniqueUserIdentifier . "='" . $main[$m][$this->uniqueUserIdentifier] . "'"),
                        $user
                    );
                    // set uid
                    $GLOBALS['BE_USER']->writelog(1, 1, 0, '', 'User %s [UID %s] updated by CSV import action', [$user['username'], $uid,]);
                    $content .= '<li><strong>' . sprintf(
                            $this->getLanguageService()->getLL('f1.tab5.userUpdated'),
                            $user[$this->uniqueUserIdentifier],
                            $userIDS[$m]
                        ) . '</strong> ' . (!empty($msg) ? implode(',', $msg) : '') . '</li>';
                    $u++;
                    ...
                    $GLOBALS['BE_USER']->writelog(
                        ...
                        0,
                        '',
                        'User %s [UID %s] created by CSV import action',
                        [$user[$this->uniqueUserIdentifier], $uid,]
                    );
                    $users[] = $user[$this->uniqueUserIdentifier];
                    $i++;
                    $content .= '<li><strong>' . sprintf(
                            $this->getLanguageService()->getLL('f1.tab5.userInserted'),
                            $user[$this->uniqueUserIdentifier],
                            $user['name'],
                            $uid
                        ) . '</strong> ' . (!empty($msg) ? implode(',', $msg) : '') . '</li>';
                ...
                $export[] = $CSV[$m];
                $content .= '<li><strong>' . sprintf(
                        $this->getLanguageService()->getLL('f1.tab5.userSkipped'),
                        $user[$this->uniqueUserIdentifier],
                        $uid
                    ) . '</strong> ' . (!empty($msg) ? implode(',', $msg) : '') . '</li>';
                $j++;
        ...
        $content = '<strong>' . sprintf(
                $this->getLanguageService()->getLL('f1.tab5.usersImported'),
                $m,
                $i,
                $u,
                $j
            ) . '</strong><ul class="import-log">' . $content . '</ul>';
    ...
    public function checkUserDataFE(&$user, &$importUser)
    {
        ...
        if (!$fatalError && !$this->enableAutoRename && strlen($user['username']) != strlen(
                preg_replace('/\s+/', '', $user['username'])
    ...
    public function setColumnNamesFromDB($tableName)
    {
        ...
        foreach ($GLOBALS['TCA'][$tableName]['columns'] as $currentColumn => $config) {
            if ((!empty($GLOBALS['TCA'][$tableName]['columns'][$currentColumn]['exclude']) && $this->getBackendUserAuthentication()->check(
                        'non_exclude_fields',
                        $tableName . ':' . $currentColumn
                    )) || empty($GLOBALS['TCA'][$tableName]['columns'][$currentColumn]['exclude'])) {
