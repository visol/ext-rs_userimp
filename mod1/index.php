				...
				if ($this->inData['settings']['enableAutoValues'] == '1') {
					...
					if (isset($this->inData['customValue'])) {
						...
						foreach ($this->inData['customValue'] as $key=>$val) {
							$myval .= !empty($val) ? $key.',':'';
							if (empty($val)) { unset($this->inData['autoval'][$key]); }
						}
...
// currently, they get deleted but i'm not sure what to do here...
				...
				/**** TAB 2 data *****/
					...
					if (isset($this->inData['settings']['importStorageFolder']) && !in_array($this->inData['settings']['importStorageFolder'],array_keys($opt))) { unset($this->inData['settings']['importStorageFolder']);}
					...
					if (isset($this->inData['settings']['uniqueIdentifier']) && !in_array($this->inData['settings']['uniqueIdentifier'],array_keys($opt))) { unset($this->inData['settings']['uniqueIdentifier']);}
